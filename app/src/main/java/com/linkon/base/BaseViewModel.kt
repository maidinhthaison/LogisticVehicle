...
abstract class BaseViewModel : ViewModel() {
    ...
    private suspend fun <T : BaseUIState> emitLoading(
        isLoading: Boolean,
    ...
    private suspend fun <T : BaseUIState> emitError(
        errorMessage: Int,
        state: MutableStateFlow<T>
    ) {
        val value = state.value.copyWith(
    ...
    private suspend fun <T, T0 : BaseUIState> emitData(data: T, state: MutableStateFlow<T0>) {
        val value = state.value.copyWith(
            data = data,
            isLoading = false,
            messageId = null
    ...
    suspend fun <T, T0 : BaseUIState> Flow<TaskResult<T>>.collectAsState(
